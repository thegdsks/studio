...
export async function generateLogo(
  ...
  const executionPromise = async (): Promise<LogoResult> => {
    ...
    const variants = images.map(img => {
      ...
      if (!b64) throw new Error("Image bytes empty");
...
function generateSvgWordmark(brand: string, vibe: string): LogoResult {
